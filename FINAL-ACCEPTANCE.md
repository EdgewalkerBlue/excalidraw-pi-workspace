...
| # | 验收项 | 状态 | 说明 |
|---|---|---|---|
| 1 | 浏览器 打开 Excalidraw | ⏳📋 | 需先完成下方「前置条件」，访问 `http://<LAN-IP>:5001` |
| 2 | 创建 3 个节点 | ⏳📋 | 浏览器 画布上操作 |
...
| 13 | 浏览器 看到 Pi 回写变更 | ⏳📋 | WebSocket 实时同步，真机确认 task-A [DONE] 与 pi-done 节点 |
...
1. **防火墙放行**（管理员 CMD）：
   ...
   ```
2. **网络**：本机以太网 IP <本机内网 IP>（WLAN 当前断开）。手机需接入同一 <LAN 网段> 的路由器（或启用本机 WLAN 并加入同网段 WiFi）。
3. Chrome 打开 `http://<LAN-IP>:5001`

## PWA 安装（浏览器）

1. 打开 `http://<LAN-IP>:5001`
